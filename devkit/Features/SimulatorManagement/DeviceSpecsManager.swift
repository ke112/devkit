...
class DeviceSpecsManager {
  ...
  /// 通过 UDID 获取 CoreSimulator 设备类型标识符。
  func getDeviceTypeIdentifier(forUDID udid: String) -> String? {
    return deviceTypeIdentifier(forUDID: udid)
  }
  ...
  private func loadDevicetypes() -> [DevicetypeEntry]? {
    guard
      let data = runProcessCaptureOutput(
        executable: simctlExecutablePath(), arguments: ["list", "devicetypes", "-j"])
    else {
  ...
  private func deviceSpecsCacheURL() -> URL? {
    do {
      let appSupport = try FileManager.default.url(
        ...
        create: true)
      return appSupport
        .appendingPathComponent("DevKit", isDirectory: true)
  ...
  private func simctlExecutablePath() -> String {
    let xcodeSimctl = "/Applications/Xcode.app/Contents/Developer/usr/bin/simctl"
    if FileManager.default.isExecutableFile(atPath: xcodeSimctl) {
      return xcodeSimctl
    }
  ...
  private func runProcessCaptureOutput(executable: String, arguments: [String]) -> Data? {
    ...
    process.arguments = executable.hasSuffix("/xcrun") ? ["simctl"] + arguments : arguments
    let pipe = Pipe()
