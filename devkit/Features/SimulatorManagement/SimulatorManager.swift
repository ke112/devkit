...
class SimulatorManager: ObservableObject, ErrorHandler {
    ...
    // 刷新失败诊断信息（用于空状态持续展示）
    @Published private(set) var refreshFailureMessage: String? = nil
    // 连续刷新失败次数（用于兜底保护）
    ...
    func start() {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                self.timer = Timer.scheduledTimer(withTimeInterval: self.refreshInterval, repeats: true) {
                    [weak self] _ in
                    guard let self, self.isActive, !self.isOperating else { return }
    ...
    private func cleanUnavailableDevices() {
        ...
        process.executableURL = URL(fileURLWithPath: configuredSimctlExecutablePath())
        process.arguments = configuredSimctlArguments(["delete", "unavailable"])
    ...
    private struct DeveloperToolCheckResult {
    ...
    private func diagnoseSimctlAvailability() -> String? {
        if let simctlPath = simctlExecutablePath {
            let fileManager = FileManager.default
            if fileManager.isExecutableFile(atPath: simctlPath) || fileManager.fileExists(atPath: simctlPath) {
                return nil
            }
            simctlExecutablePath = nil
        }

        let simctlCheck = runDeveloperToolCheck(
            executablePath: "/usr/bin/xcrun",
            arguments: ["--find", "simctl"]
        )

        if simctlCheck.succeeded {
            simctlExecutablePath = simctlCheck.output.trimmingCharacters(in: .whitespacesAndNewlines)
            return nil
        }

        let defaultXcodeSimctl = "/Applications/Xcode.app/Contents/Developer/usr/bin/simctl"
        if FileManager.default.isExecutableFile(atPath: defaultXcodeSimctl) {
            simctlExecutablePath = defaultXcodeSimctl
            PerformanceMonitor.shared.logInfo("xcrun 未找到 simctl，改用完整 Xcode 路径: \(defaultXcodeSimctl)")
            return nil
        }

        let developerDir = runDeveloperToolCheck(
            executablePath: "/usr/bin/xcode-select",
            arguments: ["-p"]
        ).output.trimmingCharacters(in: .whitespacesAndNewlines)

        if developerDir == "/Library/Developer/CommandLineTools" {
            return "无法找到 simctl。当前命令行工具路径为 /Library/Developer/CommandLineTools，Command Line Tools 不包含 iOS Simulator 管理工具。请安装并打开完整 Xcode，然后在终端执行：sudo xcode-select -s /Applications/Xcode.app/Contents/Developer，完成后点击刷新。"
        }

        let currentPath = developerDir.isEmpty ? "未知" : developerDir
        return "无法找到 simctl。当前命令行工具路径为 \(currentPath)。请确认已安装完整 Xcode，并在 Xcode > Settings > Locations 中选择 Command Line Tools，或执行：sudo xcode-select -s /Applications/Xcode.app/Contents/Developer，完成后点击刷新。"
    }

    private func runDeveloperToolCheck(executablePath: String, arguments: [String]) -> DeveloperToolCheckResult {
        let process = Process()
        let pipe = Pipe()
        ...
        do {
            ...
            return DeveloperToolCheckResult(terminationStatus: process.terminationStatus, output: output)
        } catch {
            return DeveloperToolCheckResult(terminationStatus: -1, output: error.localizedDescription)
        }
    }

    private func configuredSimctlExecutablePath() -> String {
        simctlExecutablePath ?? "/usr/bin/xcrun"
    }

    private func configuredSimctlArguments(_ arguments: [String]) -> [String] {
        simctlExecutablePath == nil ? ["simctl"] + arguments : arguments
    }

    /// 刷新设备列表
    func refreshDevices() {
        guard isActive, !isOperating, !isRefreshing else { return }
        isRefreshing = true
    ...
    private func executeRefreshWithTimeout() -> Bool {
        if let diagnosticMessage = diagnoseSimctlAvailability() {
            PerformanceMonitor.shared.logInfo("simctl 不可用: \(diagnosticMessage)")
            DispatchQueue.main.async { [weak self] in
                guard let self else { return }
                if self.deviceCache.isEmpty {
                    self.refreshFailureMessage = diagnosticMessage
                }
                self.handleError(SimulatorError.commandExecutionFailed(diagnosticMessage))
            }
            return !deviceCache.isEmpty
        }
        ...
        if jsonSuccess, let devices = devicesFromJson {
            ...
            if textSuccess, let devices = devicesFromText {
                ...
                let message = "无法获取模拟器列表。请确认已安装完整 Xcode，且 xcode-select 指向 Xcode.app/Contents/Developer。"
                DispatchQueue.main.async { [weak self] in
                    self?.refreshFailureMessage = message
                    self?.handleError(SimulatorError.commandExecutionFailed(message))
                }
                return false
            }
        }
    ...
    private func tryRefreshWithJsonFormat(timeoutSeconds: Double) -> (Bool, [SimulatorDevice]?) {
        ...
        process.executableURL = URL(fileURLWithPath: configuredSimctlExecutablePath())
        process.arguments = configuredSimctlArguments(["list", "devices", "-j"])
        process.standardOutput = pipe
        process.standardError = pipe
        ...
        do {
            ...
            if process.terminationStatus != 0 {
                let output = String(data: data, encoding: .utf8) ?? ""
                PerformanceMonitor.shared.logDebug(
                    "JSON 格式刷新失败，退出码 \(process.terminationStatus): \(output)")
                return (false, nil)
            }
    ...
    private func tryRefreshWithTextFormat(timeoutSeconds: Double) -> (Bool, [SimulatorDevice]?) {
        ...
        process.executableURL = URL(fileURLWithPath: configuredSimctlExecutablePath())
        process.arguments = configuredSimctlArguments(["list", "devices"])
        process.standardOutput = pipe
        process.standardError = pipe
        ...
        do {
            ...
            if process.terminationStatus != 0 {
                let output = String(data: data, encoding: .utf8) ?? ""
                PerformanceMonitor.shared.logDebug(
                    "文本格式刷新失败，退出码 \(process.terminationStatus): \(output)")
                return (false, nil)
            }
    ...
    private func shouldSortDevice(
        ...
        let releaseYear1 = Self.deviceReleaseYear(from: device1.name)
        let releaseYear2 = Self.deviceReleaseYear(from: device2.name)
        ...
        if device1.deviceType != device2.deviceType {
            if device1.deviceType == .iPhone {
                return true
            }
            if device2.deviceType == .iPhone {
                return false
            }
            if device1.deviceType == .iPad {
                return true
            }
            if device2.deviceType == .iPad {
                return false
            }
        }

        let pixelCount1 = Self.resolutionPixelCount(from: device1.resolution)
        let pixelCount2 = Self.resolutionPixelCount(from: device2.resolution)
    ...
    static func deviceReleaseYear(from name: String) -> Int {
        let normalizedName = name.replacingOccurrences(
            of: #"\s+\((?:8|12|16)GB\)$"#,
            with: "",
        ...
        if let releaseYear = releaseYearByDeviceName[normalizedName] {
            return releaseYear
        }
    ...
    static func resolutionPixelCount(from resolution: String) -> Int {
        let dimensions = resolution.split(separator: "*")
    ...
    private func getInstalledRuntimes() -> [String] {
        ...
        process.executableURL = URL(fileURLWithPath: configuredSimctlExecutablePath())
        process.arguments = configuredSimctlArguments(["list", "runtimes", "-j"])
        process.standardOutput = pipe
    ...
    func recreateDevice(
        ...
        guard let device = devices.first(where: { $0.udid == udid }) else {
            handleError(SimulatorError.deviceNotFound(udid))
            completion?(false)
            return
        }

        guard !device.isPlaceholder else {
            handleError(SimulatorError.commandExecutionFailed("无法重建：该设备信息不完整，请刷新后重试"))
            completion?(false)
            return
        }

        guard let deviceTypeIdentifier = DeviceSpecsManager.shared.getDeviceTypeIdentifier(forUDID: udid) else {
            handleError(SimulatorError.commandExecutionFailed("无法重建：未找到该设备的机型标识"))
            completion?(false)
            return
        }
        ...
        isOperating = true
        operatingDeviceUDID = udid
        operatingDeviceAction = .recreating
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            if device.state == "Booted" {
                succeeded = self.executeSimctlCommand(arguments: ["shutdown", udid])
                if succeeded {
                    Thread.sleep(forTimeInterval: 0.5)
                }
            }
            ...
            if succeeded {
                succeeded = self.executeSimctlCommand(arguments: ["delete", udid])
            }

            let newUDID: String?
            if succeeded {
                newUDID = self.createSimulatorDevice(
                    name: device.name,
                    deviceType: deviceTypeIdentifier,
                    runtime: device.runtime
                )
                succeeded = newUDID != nil
            } else {
                newUDID = nil
            }
            ...
            DispatchQueue.main.async {
                if let newUDID {
                    let recreatedDevice = SimulatorDevice(
                        udid: newUDID,
                        name: device.name,
                        state: "Shutdown",
                        runtime: device.runtime,
                    ...
                    self.devices = self.devices.map { $0.udid == udid ? recreatedDevice : $0 }
                    self.deviceGroups = self.deviceGroups.map { group in
                        var updatedGroup = group
                        updatedGroup.devices = group.devices.map {
                            $0.udid == udid ? recreatedDevice : $0
                        }
                        return updatedGroup
                    }
                }
    ...
    private func executeSimctlCommand(arguments: [String], timeoutSeconds: Double = 30.0) -> Bool {
        ...
        process.executableURL = URL(fileURLWithPath: configuredSimctlExecutablePath())
        process.arguments = configuredSimctlArguments(arguments)
    ...
    private func getRuntimeDeletionIdentifier(for runtimeIdentifier: String) -> String {
        ...
        process.executableURL = URL(fileURLWithPath: configuredSimctlExecutablePath())
        process.arguments = configuredSimctlArguments(["runtime", "list", "-j"])
            process.standardOutput = pipe
            process.standardError = FileHandle.nullDevice
    ...
    ///   - deviceType: 设备类型标识符
    ///   - runtime: runtime 标识符
    /// - Returns: 创建成功后的 UDID，失败时返回 nil
    private func createSimulatorDevice(name: String, deviceType: String, runtime: String) -> String? {
        do {
            let process = Process()
            let pipe = Pipe()
            
        process.executableURL = URL(fileURLWithPath: configuredSimctlExecutablePath())
        process.arguments = configuredSimctlArguments(["create", name, deviceType, runtime])
            process.standardOutput = pipe
            process.standardError = pipe
            ...
            if process.terminationStatus == 0 {
                ...
                guard let udid else {
                    print("创建失败 \(name): simctl 未返回新设备 UDID")
                    return nil
                }
                print("创建成功: \(name) (UDID: \(udid))")
                return udid
            } else {
                let data = pipe.fileHandleForReading.readDataToEndOfFile()
